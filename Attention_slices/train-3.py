...
def train_model(model, train_loader, epoch, num_epochs, optimizer, writer, current_lr, counting,log_every=100):
    _ = model.train()
    ...
    scores = pd.DataFrame(pd.concat([pd.DataFrame(att_scores), pd.DataFrame(inds)], axis =1))
    scores.to_csv('scores2_train{}'.format(counting))
    return train_loss_epoch, train_auc_epoch


def evaluate_model(model, val_loader, epoch, num_epochs, writer, current_lr, counting,log_every=20):
    _ = model.eval()
    ...
    scores = pd.DataFrame(pd.concat([pd.DataFrame(att_scores), pd.DataFrame(inds)], axis =1))
    scores.to_csv('scores2_valid{}'.format(counting))
...
def run(args):
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_auc = train_model(
            mrnet, train_loader, epoch, num_epochs, optimizer, writer, current_lr, counting, log_every)
        val_loss, val_auc = evaluate_model(
            mrnet, validation_loader, epoch, num_epochs, writer, counting, current_lr)
        counting = counting+1
