...
def train_model(model, train_loader, epoch, num_epochs, optimizer, writer, current_lr, log_every=100):
    ...
    for i, (image, label, weight) in enumerate(train_loader):
        ...
        print(label)
        print(weight)
        loss = torch.nn.BCEWithLogitsLoss(weight=weight)(prediction[0], label)
        print(loss)
        loss.backward()
        optimizer.step()
        ...
        print(label.numpy())
        y_trues.append(label.numpy()[0])
        y_preds.append(probas.detach().numpy()[0][0])
...
def evaluate_model(model, val_loader, epoch, num_epochs, writer, current_lr, log_every=20):
    ...
    for i, (image, label, weight) in enumerate(val_loader):
        ...
        y_trues.append(label.numpy()[0][0])
        y_preds.append(probas.detach().numpy()[0][0])
